...
interface RegisterWaiverClientPageProps {
  selectedIdsParam: string
}

export default function RegisterWaiverClientPage({
  selectedIdsParam,
}: RegisterWaiverClientPageProps) {
  const [accepted, setAccepted] = useState(false)
  const [signature, setSignature] = useState("")

  const canContinue = accepted && signature.trim().length > 0
  const registerPaymentHref = canContinue
    ? {
        pathname: "/register/payment",
        query: selectedIdsParam ? { classIds: selectedIdsParam } : {},
      }
    : "#"
  const registerClassesHref = {
    pathname: "/register/classes",
    query: selectedIdsParam ? { classIds: selectedIdsParam } : {},
  }
